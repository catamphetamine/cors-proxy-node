...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var requestLocation = parseURL(getFromOriginForHttpRequest(req) + req.url);
    ...
    if (corsAnywhere.fromOriginBlacklist.indexOf(fromOrigin) >= 0) {
      ...
      res.end('The origin "' + fromOrigin + '" was blacklisted by the operator of this proxy.');
      return;
    }
    ...
    if (corsAnywhere.fromOriginWhitelist.indexOf(fromOrigin) < 0) {
      if (corsAnywhere.toOriginWhitelist.indexOf(toOrigin) < 0) {
        res.writeHead(403, 'Forbidden', cors_headers);
        res.end('The origin "' + fromOrigin + '" was not whitelisted by the operator of this proxy.');
        return;
      }
    }
    ...
    if (rateLimitMessage) {
      ...
      res.end('The origin "' + fromOrigin + '" has sent too many requests.\n' + rateLimitMessage);
      return;
    }
